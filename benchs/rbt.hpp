...
struct subtree_value_empty_t{
 template<typename A> subtree_value_empty_t(A...){}
};
...
template <typename Key, typename Value, typename Compare = std::less<Key>, typename SubtreeValue = subtree_value_empty_t> class rb_tree {
 ...
 using key_t = Key;
 using value_t = Value;
 using subtree_value_t = SubtreeValue;

 public:
 // node type node_t is inherited from Value, and is the type of the data stored on the node
 struct node_t : public Value, public SubtreeValue {
  Key key;                              // key
  bool color;                           // color of parent link
  node left = nullptr, right = nullptr; // links to left and right subtrees
  int _subtree_size;                    // subtree count
  bool modified, delete_flag;

  node_t(Key const& key, Value const& val, bool color, int subtree_size) 
      : Value(val),
        SubtreeValue(key, val),
        key(key),
        color(color),
        left{nullptr},
        right{nullptr},
  ...
  node_t(node_t const& n) : Value(n), SubtreeValue(n) {
   key = n.key;
   color = n.color;
   if (n.left) left = new node_t(*n.left);
   if (n.right) right = new node_t(*n.right);
   _subtree_size = n._subtree_size;
   modified = n.modified;
...
private : node root; // root of the BST
 ...
 int subtree_size(node x) const {
  if (x == nullptr) return 0;
  return x->_subtree_size;
 }
 ...
 *************************************************************************/
 ...
 public:
 /*************************************************************************
  *  Find the n-th node in the ascending order.
  *************************************************************************/

 node find_n_th(int n) const {
  if ((n<0) ||(n >= size())) return nullptr;
  auto x = root;
  while (x && (subtree_size(x->left) != n)) {
   x = (subtree_size(x->left) > n) ? x->left : x->right;
  }
  return x;
 }
...
/*************************************************************************
 ...
 // throws rbt_insert_error if the key is already present
 ...
 node insert(node h, Key const& key, Value const& val) {
  ...
  h->_subtree_size = subtree_size(h->left) + subtree_size(h->right) + 1;
  h->SubtreeValue::operator=(SubtreeValue{h});
 ...
 node rotateRight(node h) {
  ...
  x->right->color = RED;
  x->_subtree_size = h->_subtree_size;
  h->_subtree_size = subtree_size(h->left) + subtree_size(h->right) + 1;
  x->SubtreeValue::operator=(static_cast<SubtreeValue const&>(*h));
  h->SubtreeValue::operator=(SubtreeValue{h});
  h->modified = true;
  x->modified = true;
  return x;
 ...
 node select(node x, int k) const {
  _assert(x != nullptr);
  _assert(k >= 0 && k < subtree_size(x));
  int t = subtree_size(x->left);
  if (t > k)
 ...
 int rank(Key const& key, node x) const {
  ...
  if (compare(key,x->key))
   ...
   return 1 + subtree_size(x->left) + rank(key, x->right);
  else
   return subtree_size(x->left);
 }
 ...
 */
 ...
 bool isSizeConsistent(node x) {
  if (x == nullptr) return true;
  if (x->_subtree_size != subtree_size(x->left) + subtree_size(x->right) + 1) return false;
